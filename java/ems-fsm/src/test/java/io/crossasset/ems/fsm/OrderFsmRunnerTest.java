 ...
 *
 * Covers the four categories the advisor called out:
 *   (1) Guarded PENDING_REPLACE branches (NEW vs REPLACED restore)
 *   (2) Guarded PENDING_CANCEL three-way restore
 *   (3) u64 qty arithmetic in PartialFill / FullFill
 *   (4) Totality — every (state, event) either transitions or returns noTransition
 */
class OrderFsmRunnerTest {
    ...
    void replaceRejected_fromNew_returnsToNew() {
        ...
        ctx = ctx.with(ctx.orderId(), ctx.clOrdId(), ctx.origClOrdId(), ctx.instrumentId(),
            ctx.side(), ctx.orderQty(), ctx.price(), ctx.cumQty(), ctx.leavesQty(),
            ctx.account(), ctx.tif(), ctx.traceId(), ctx.initialOrderId(),
            ctx.preCancelStatus(), "0");
    ...
    void partialFill_whilePendingReplace_updatesQtyAndLeavesReplace() {
        ...
        ctx = ctx.with(ctx.orderId(), ctx.clOrdId(), ctx.origClOrdId(), ctx.instrumentId(),
            ctx.side(), ctx.orderQty(), ctx.price(), ctx.cumQty(), ctx.leavesQty(),
            ctx.account(), ctx.tif(), ctx.traceId(), ctx.initialOrderId(),
            ctx.preCancelStatus(), "0");  // in pending replace
        ...
        assertFalse(result.isNoTransition());
        assertEquals(PARTIALLY_FILLED, result.newState(), "Fill during pending-replace must land in PARTIALLY_FILLED");
    ...
    void totality_allStateEventPairsNeverThrow() {
        ...
        for (OrderFsmState state : OrderFsmState.values()) {
            for (OrderFsmEvent event : OrderFsmEvent.values()) {
                ...
                assertNotNull(result, "transition() must never return null for state=" + state + " event=" + event);
    ...
    private static Object pickPayload(OrderFsmEvent event) {
        return switch (event) {
            case ReplaceRequested -> new OrderFsmPayloads.ReplaceRequestedPayload("cl-002", 100L, (Long) null);
    ...
    void cancelAccepted_emitsCascadeToRouteFsm() {
        ...
        boolean hasCascade = result.effects().stream()
            .anyMatch(e -> e instanceof OrderFsmEffect.EmitEvent ee
