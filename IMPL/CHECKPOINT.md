...
- **Last completed task:** 18.22 — trading-grade grouping: signedQty expression, same-or-MULTI badge (`aggregates.ts`), server-side WAP (BlotterPublisher accumulators, record-before-delegate + reject rollback), TIF/ordType columns, weighted-mean group aggregate (`["weighted mean", ["cumQty"]]` — 3.8 serde needs the array-wrapped weight). 18.21 before it: cross-asset DemoUniverse (13 instruments / 6 classes / per-class lots+venues+FX-converted P&L).
- **In-progress task:** _(none — next: 18.27 multi-select linking, 18.28 notional, 18.29 issuer grouping, 18.30 currency model, then 18.23 column picker → 18.24 docking → 18.25 audit trail → 18.26 demo telemetry. 18.31 backend-restart recovery DONE: recovery.ts watchdog + sessionStorage auto-logon, verified by kill/restart probe — desktop self-recovers with a fresh session, no manual refresh.)_
- **Session-limit pause (2026-06-11):** user token budget exhausted; cron scheduled to resume 2026-06-12 00:05 local. Quiet-edge/preview/probe processes were killed at wrap-up. Lessons this session: a failed `viewer.restore()` poisons subsequent restores in that viewer; kill demo edges by explicit java PID (gradle wrapper kill leaves the child alive).
- **WIP branch:** main
...
- **Total progress:** **149 of 189 tasks [x]** (78.8%). Phase 18 addenda: 18.21 + 18.22 done; queued — 18.27 multi-select linking, 18.23 column picker, 18.24 dockable layout, 18.25 audit-trail viewer, 18.26 demo telemetry (OTel gap). MVP v0, Phase 16, v1 build-out complete.
