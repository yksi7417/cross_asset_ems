...
- **Last completed task:** 18.17 — Desktop UX round 2 `(f5d8908)` (user feedback). Names everywhere (figi→name cache over /instruments), linked blotter (order→routes, route→fills via perspective-click; LINK toggle + chips), fills hidden by default, ctrl+click multi-select + right-click context menus (batch ready/route/cancel, aggregate→basket, cancel routes; toasts), curated default columns (keys must stay visible — perspective-click rows carry only visible columns). Demo re-recorded (30s + linked frame). All headless-verified.
- **In-progress task:** _(none — goal complete + 18.15–18.17 addenda done)_
- **WIP branch:** main
...
- **Total progress:** **144 of 180 tasks [x]** (80.0%). Phase 18 (incl. 18.15–18.17 addenda) + 11.17 complete. MVP v0, Phase 16, v1 build-out complete; Phase 18 underway.
