 ...
 *
 * Records a multi-step scenario (forward pass) then re-runs each step from scratch
 * (replay pass) and asserts that transition() produces identical outputs every time.
 * This verifies that the generated FSM is a pure function with no hidden state.
 */
class FsmReplayDeterminismTest {
    ...
    private static ReplayEntry capture(
        ...
        return new ReplayEntry(state, event, ctx, payload,
            result.newState(), result.isNoTransition(),
            nc.orderId(), nc.clOrdId(), nc.origClOrdId(), nc.instrumentId(),
            nc.side(), nc.orderQty(), nc.price(),
            nc.cumQty(), nc.leavesQty(),
            nc.account(), nc.tif(), nc.traceId(), nc.initialOrderId(),
            nc.preCancelStatus(), nc.preReplaceStatus());
    }
    ...
    private static void replayAndAssert(List<ReplayEntry> log) {
        for (ReplayEntry e : log) {
            ...
            if (!e.expectedNoTransition()) {
                ...
                assertEquals(e.expectedPreReplaceStatus(), nc.preReplaceStatus(), tag + ": preReplaceStatus");
            }
        }
    }
    ...
    static OrderFsmContext minimal() {
        return new OrderFsmContext(
            "order-001", "cl-001", null, "BBG000BLNNH6",
            1, 100L, null, 0L, 100L,
            "ACC-1", 0, 0L, "order-001", null, null);
    }
    ...
    void fillPath_isReplayDeterministic() {
        ...
        state = r1.newState(); ctx = r1.newContext();
        ...
        state = r2.newState(); ctx = r2.newContext();
        ...
        state = r3.newState(); ctx = r3.newContext();
