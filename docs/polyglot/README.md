...
| 4 | **[Rust slice](../superpowers/plans/2026-07-30-polyglot-04-rust-slice.md)** | Full slice in Rust. Rust gate + conformance green. | 2, 3 | **in progress** — `ems-core`, `ems-transport`, `ems-aaa`, `ems-slice` |
| 5 | **[C++ slice](../superpowers/plans/2026-07-30-polyglot-05-cpp-slice.md)** | Full slice in C++, replacing the stubs. C++ gate + conformance green. | 2 | **in progress** — `ems-core`, `ems-transport`, `ems-aaa`, `ems-it` |
...
rust/                  # cargo workspace: ems-core, ems-transport, ems-aaa, ems-slice
...
`ems-transport` (the ADR 0006 seam), **3** `ems-aaa` (session lookup + entitlement decision, the
first rejection path in the corpus). The validation pipeline is next, then the order FSM — which
brings the Rust codegen emitter with it.
