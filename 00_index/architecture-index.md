...
- [[arch-deployment]] — **CI/CD + blue/green + per-pod multi-tenancy**. Four environment tiers (Dev / QA on Docker Compose + GitHub Actions, UAT-1/UAT-2 on smaller AWS, PROD-1/PROD-2 on Aeron-tuned AWS per the Aeron Cloud AWS performance testing guidance). Per-pod blue/green pairs (dedicated for Tier-1 clients, shared for Tier-2/3). **High-precision switchover protocol** using a cluster-of-clusters active lease + fence tokens + Aeron Archive position-precise handoff — guarantees no dual-live, no duplicates, no missed messages, sub-second client-perceived window. UAT pod mimicry replays any PROD pod's manifest (compliance lists / allocation templates / broker codes / per-client rules) for client-specific pre-deployment validation. Window-aware switchover per asset class (crypto has no maintenance window, FX prefers weekend, futures prefers Globex pause).
...
- [[arch-regulatory-reporting-service]] — per-trade + periodic reports across **all jurisdictions** the firm operates in: US (TRACE, MSRB, CFTC SDR, FINRA CAT, FICC, Fed, large-trader), EU (MiFIR RTS 22 + APA post-trade transparency, EMIR, SFTR, CSDR, MAR STORs, AIFMD, RTS 27/28), UK (UK MiFIR / EMIR / SFTR), APAC (HKEX/SFC, MAS, JFSA, ASIC, CSRC), cross-border (FATCA / CRS / sanctions). Determination logic, per-regulator profiles, deadline tracking, ack/nack lifecycle, retry policy, amendment/void protocol. **Concurrent multi-jurisdiction reporting** of single trades documented.

## Compliance Regimes — Cross-Cutting

- [[arch-jurisdictional-compliance]] — per-jurisdiction **non-reporting obligations** matrix: LEI mandates, algo/HFT flagging (RTS 6/7), decision-maker capture, OFAC/EU/UN sanctions screening, MiFID II pre-trade transparency + SI quote publication, tick-size regime (RTS 11), trader certification (Article 25), clock synchronization (RTS 25), phone recording (Article 16(7)), order record-keeping retention by regime, MAR surveillance (with STOR filing), CSDR settlement-discipline tracking, best-ex policy compliance. Consumed by validator / compliance / surveillance / TCA / reporting; matrix is versioned reference data.
- [[arch-best-execution]] — firm-wide **best-execution obligation + audit chain**. Per-jurisdiction obligations (MiFID II "all sufficient steps" + RTS 27/28, US "reasonable diligence" FINRA 5310, HK SFC 3.2, SG MAS, AU FOFA, JP JFSA, CA UMIR). Per-order best-ex record tying policy → recommendation → SOR selection → execution → measurement → reporting. Broker selection audit (wheel selection rationale + manual override capture). Execution slippage evidence. Periodic reports (RTS 28, internal BEC). Client on-demand reports. **End-to-end audit reconstruction** example demonstrating subpoena-grade reproducibility.
...
- [[arch-configuration-service]] — **dedicated configuration service**: push-without-bounce value delivery to running components via a config multicast bus + per-box local cache agents. Atomic snapshot swap at message boundaries (one message → one config view). **Replay-deterministic** by construction: keys are never deleted, only archived with a locked `default_value`, so historical events resolve to their config-at-event-time even months later. Nine-level scope cascade (global → environment → region → pod → asset_class → firm → desk → user → order). **Distinct author/approver tag families** with validator-enforced `author.identity != approver.identity` for genuine two-person control; window-aware in UAT/PROD using the [[arch-resilience-24x7]] maintenance-window table; emergency loosening requires a fourth signature. ConfigChanged events flow through [[arch-observability|OpenSearch]] for search and point-in-time queries. Per-pod isolation; UAT pod mimicry delivered as a replay of the target PROD pod's ConfigChanged stream with secrets substituted. Schema (keys, types) ships with code; **values** ship at runtime — separate disciplines.
