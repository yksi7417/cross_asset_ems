...
RBLD ("Rebalance / Block-Distribute") is an FX automation pattern for **portfolio-rebalance flows**: many small orders generated by a portfolio rebalance (often from AIM or another buy-side OMS) get auto-aggregated, netted, and distributed across venues by rule rather than by hand.
...
- A batch of FX orders staged from AIM with `extension.batch_kind = RBLD` (or equivalent firm convention).
- The desk's RBLD rule is bound and `enabled`.

## Actors

- Buy-side OMS (AIM) — source.
...
```mermaid
flowchart LR
  AIM[AIM batch upload] --> S[Stage many orders]
  S --> A{RBLD Rule}
...
1. AIM batch arrives; orders enter [[arch-order-staged|staged state]] tagged with the RBLD batch ID.
...
- **Partial AIM upload (timing).** If AIM uploads in waves over several minutes, the rule needs a `batch_close` signal (explicit) or a quiet-window timeout. Mis-close → an order misses the netting bucket, executes alone.
...
- [[auto-route]] · [[fx-automation-tradebest]] · [[tsox-aim-to-fxem]] · [[auto-route-fixing-aim]]
