...
| Item | Outcome |
|---|---|
| T-9 — close the codec edge-case gap between the reference and the ports | Done — the audit inverted the premise: the ports tested four hostile-input cases Java did not, and one hid a real acceptance divergence. Java's parser now rejects any `\uXXXX` escape in D800–DFFF (matching both ports; astral characters travel as literal UTF-8); the four tests exist in Java; the lone-surrogate test was watched failing before the fix; conformance stayed 24/24. Ids were already at parity, and Java's `negativeSeedIsRejected` is unrepresentable in the ports by type. |
