...
- [ ] **1.7b** C++ FSM transition impl: fill `transition()` stubs in `cpp/fsm/generated/*.hpp`; compile-verify via CMake (sonnet) ← blocks: C++ simulator path
...
- [ ] **1.11** Replay determinism test harness (single FSM, replay log slice) (sonnet) ← blocks: 1.7
- [ ] **1.12** Identity chaining stamping into FSM events per [[arch-identity-chaining]] (sonnet) ← blocks: 1.7
...
- [ ] **2.3** Aeron channel layout doc and code (sonnet) ← blocks: 2.1
- [ ] **2.4** Sequence recovery service skeleton (sonnet) ← blocks: 2.2
...
- [ ] **3.2** Event log writer (append-only, fsync discipline) (gemma) ← blocks: 3.1
...
- [ ] **3.7** Configuration service per [[arch-configuration-service]] (sonnet) ← blocks: 3.1, 3.5
...
- [ ] **4.1** Symbology service (FIGI + licensed secondaries) per [[arch-symbology-figi]] (sonnet)
...
- [ ] **7.1** Staged Order Manager per [[arch-order-staged]] (sonnet) ← blocks: 1.2, 4.19, 6.2
