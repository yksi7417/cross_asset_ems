 ...
 *
 * <p>The MultiLeg guards use integer arithmetic (legs_filled + 1 + legs_rejected + legs_canceled ==
 * total_legs) combined with AND/OR. This class verifies the operator precedence and boundary
 * conditions the advisor flagged as unverified after task 1.7.
 *
 * <p>Also includes one smoke transition per FSM not covered by OrderFsmRunnerTest (Route, SOR,
 * VenueSession).
 */
class MultiLegFsmRunnerTest {
  ...
  static MultiLegFsmContext ctx(
      String executionMode, long totalLegs, long legsFilled, long legsRejected, long legsCanceled) {
    return new MultiLegFsmContext(
        "ml-001", "PAIRS", executionMode, totalLegs, legsFilled, legsRejected, legsCanceled, null);
  }
  ...
  void sorFsm_smoke_sent_strategyDecided_selfLoop() {
    var ctx =
        new SorFsmContext(
            ...
            "TWAP");
    var result =
        SorFsmRunner.transition(SorFsmState.SENT, SorFsmEvent.SorStrategyDecided, ctx, null);
  ...
  void venueSessionFsm_smoke_disconnected_connectRequested_transitionsToConnecting() {
    var ctx = new VenueSessionFsmContext("vs-001", 1L, 1L, 30L, false, 0L, 0L, "XNAS");
    var result =
        VenueSessionFsmRunner.transition(
            VenueSessionFsmState.DISCONNECTED, VenueSessionFsmEvent.ConnectRequested, ctx, null);

    assertFalse(
        result.isNoTransition(),
        "VenueSessionFsm: DISCONNECTED + ConnectRequested should transition");
