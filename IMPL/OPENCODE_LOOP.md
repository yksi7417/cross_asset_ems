...
2.3  (sonnet)  Aeron channel layout doc + code        (← 2.1 ✓)
2.4  (sonnet)  Sequence recovery service skeleton     (← 2.2 ✓)
2.7  (sonnet)  Schema evolution test                  (← 2.1 ✓)
3.2  (gemma)   Event log writer                       (← 3.1 ✓)
3.3  (sonnet)  Stream-id partitioning                 (← 3.1 ✓)

# Phase 1 — prereqs met
1.8  (gemma)   Unit test generator YAML → ~5000 tests (← 1.7 ✓)
1.12 (sonnet)  Identity chaining stamping             (← 1.7 ✓)
